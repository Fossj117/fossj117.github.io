...
* Something related to having multiple tags arranged in space, say in a grid. Scanning each one sends request to corresponding server, updating some piece of art that is visible..online? in physical space somewhere else? 
* Relatedly, some kind of low fidelity communication channel? Maybe there are multiple input points? 
* Leave a message for the next person. Read what the most recent message was.